 ...
 */
class CostInjector constructor(classContext: ClassContext, api: Int, visitor: MethodVisitor, access: Int, name: String, descriptor: String, properties: Properties, isKt: Boolean, method: Method) : Injector(classContext, api, visitor, access, name, descriptor, properties, isKt, method) {
    private val currentTime = newLocal(LONG_TYPE)
    private val castTime = newLocal(LONG_TYPE)
    override fun onMethodEnter() {
        if (!properties.enableCost) return
        if (properties.methodCostMatcher.isNotEmpty() && !("${name}${descriptor}").matches(Regex(properties.methodCostMatcher))) return
        mv.visitLabel(Label())
    ...
    override fun onMethodExit(opcode: Int) {
        ...
        mv.visitLabel(Label())
        mv.visitLdcInsn(properties.tagCost)
        mv.visitLdcInsn("${Log.SQUARE_LEFT} ${String.format(header, Language.get(properties.useEnglish, Language.METHOD))}: ${name}(${simpleName}.${sourceExtension}:${curLineNumber})")
        mv.visitMethodInsn(INVOKESTATIC, C_LOG, properties.levelCost, "(Ljava/lang/String;Ljava/lang/String;)I", false)
        mv.visitInsn(POP)
