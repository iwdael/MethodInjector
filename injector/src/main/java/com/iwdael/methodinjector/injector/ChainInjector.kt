...
import com.iwdael.methodinjector.utils.Language
 ...
 */
class ChainInjector constructor(classContext: ClassContext, api: Int, visitor: MethodVisitor, access: Int, name: String, descriptor: String, properties: Properties, isKt: Boolean, method: Method) : Injector(classContext, api, visitor, access, name, descriptor, properties, isKt, method) {

    override fun onMethodEnter() {
        if (!properties.enableChain) return
        if (properties.methodChainMatcher.isNotEmpty() && !("${name}${descriptor}").matches(Regex(properties.methodChainMatcher))) return
        ...
        mv.visitLabel(Label())
        mv.visitLdcInsn(properties.tagChain)
        mv.visitLdcInsn("${Log.SQUARE_LEFT} ${String.format(header, Language.get(properties.useEnglish, Language.METHOD))}: ${name}(${simpleName}.${sourceExtension}:${method.lineNumber})")
        mv.visitMethodInsn(INVOKESTATIC, C_LOG, properties.levelChain, "(Ljava/lang/String;Ljava/lang/String;)I", false)
        mv.visitInsn(POP)
        ...
        argumentTypes.forEachIndexed { index, type ->
            ...
            mv.visitInsn(DUP)
            mv.visitMethodInsn(INVOKESPECIAL, C_STRING_BUILDER, C_INIT, "()V", false)
            mv.visitLdcInsn("${Log.SQUARE_LEFT} ${String.format(header, (Language.get(properties.useEnglish, Language.ARG) + "[" + method.args[index] + "]"))}: ")
            mv.visitMethodInsn(INVOKEVIRTUAL, C_STRING_BUILDER, C_APPEND, "(Ljava/lang/String;)Ljava/lang/StringBuilder;", false)
            loadArg(index)
