...
import com.iwdael.methodinjector.utils.isKt
...
class InjectClassVisitor(private val context: ClassContext, visitor: ClassVisitor, private val properties: Properties, private val sourceFile: File?, private val asmApi: Int) : ClassVisitor(asmApi, visitor) {
    ...
    init {
        ...
        Logger.log(methods.toString())
    }

    override fun visitMethod(access: Int, name: String, descriptor: String, signature: String?, exceptions: Array<String>?): MethodVisitor {
        val visitor = super.visitMethod(access, name, descriptor, signature, exceptions)
        val injector = ChainInjector(context, asmApi, visitor, access, name, descriptor, properties, sourceFile.isKt, getMethod(name, descriptor))
        return CostInjector(context, asmApi, injector, access, name, descriptor, properties, sourceFile.isKt, getMethod(name, descriptor))
    }
    ...
    inner class GetMethodVisitor(api: Int, methodVisitor: MethodVisitor?, access: Int, name: String?, descriptor: String?) : AdviceAdapter(api, methodVisitor, access, name, descriptor) {
        private val method = Method(name, 0, descriptor)
